...
const CoffeeDetail = ({ menu, addToCart }: CoffeeDetailProps) => {
    ...
    return (
        <div className="border rounded-lg p-1 shadow-md hover:shadow-xl transition bg-white flex flex-col">
            {/* Menu */}
            <img
            ...
            <div className="flex flex-col mt-2">
                ...
                <div className="flex gap-2 mt-1 justify-center">
                    {["0%", "50%", "100%", "150%"].map((level) => (
                        <button
                            key={level}
                            className={`px-2 py-1 text-xs rounded-md border ${sweetness === level
                                    ? "bg-black text-white"
                                    : "bg-white text-black"
                                }`}
                            onClick={() => setSweetness(level)}
                        >
                            {level} {/* แสดงค่าที่เลือก */}
                        </button>
                    ))}
                </div>
            </div>
